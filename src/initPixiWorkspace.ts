...
export class PixiWorkspaceInitializer implements IProjectInitializer {
  ...
  constructor(cache: typeof Cache, pixiProjects: Pixi[]) {
    this.pixiProjects =
      pixiProjects.length > 0 ? pixiProjects : [new Pixi("pseudo_project")];
    this.configCollector = new ProjectConfigurationCollector(
      cache,
      this.pixiProjects
    );
  }
  ...
  public async initPixiWorkspace(): Promise<void> {
    ...
    const projectConfig =
      await this.configCollector.collectProjectConfiguration();
    if (!projectConfig) {
      return;
    }
  ...
  private async initializeProject(config: ProjectConfig): Promise<void> {
		...
		args.push(PixiToolPath);
		args.push(PixiCommand.init);
		const newdir = config.parentDir.fsPath + "/" + config.projectName;

		// if newdir doesnt exist, create it
		if ( !fs.existsSync(newdir) ) {
			log.info("Creating new directory: ", newdir);
			fs.mkdirSync(newdir);
		}

		args.push(newdir);
		if (config.projectType == PixiProjectType.Pyproject) {
			args.push("--format pyproject");
		}
		if (config.platforms) {
    ...
    try {
     	log.debug("Initializing Pixi project with config: ", config);
			log.debug("Running command: ", args.join(" "));

			const result = await execShellWithTimeout(args.join(" "), 10000);
			log.info("Pixi project initialized successfully: ", result);
			
    } catch (error) {
      vscode.window.showErrorMessage(
        `Failed to initialize Pixi workspace: ${error}`
      );
    }
  }
}
...
class ProjectConfigurationCollector {
  ...
  constructor(cache: any, pixiProjects: Pixi[]) {
	...
	/**
	 * Main driver function to collect project configuration.
	 * 
	 * DETAILS:
	 * 1. Prompt user to select a folder to initialize the project in.
	 * 2. Prompt user to enter a project name.
	 * 3. Prompt user to select a project type.
	 * 4. Prompt user to select platforms.
	 * 5. Prompt user to select channels.
	 * 
	 * */
  public async collectProjectConfiguration(): Promise<
  ...
  private async promptForProjectName(
  ...
  private async chooseProjectType(): Promise<string> {
    ...
    const projectTypes = Object.values(PixiProjectType).map((type) => ({
      label: type,
    }));

    const selectedProjectType = await showQuickPick({
      title: `Select Project Type${
        defaultProjectType ? ` (Default: ${defaultProjectType})` : ""
      }`,
  ...
  private async getPlatforms(): Promise<string[]> {
    const userPlatform = await this.pixiProjects[0]
      .getPixiInfo()
    ...
    const previouslySelectedPlatforms: string[] = this.cache
    ...
    const selectedPlatforms = await showQuickPick({
      ...
      this.cache.put("selectedPlatforms", [
        ...previouslySelectedPlatforms,
        ...platforms,
      ]);
      return platforms;
  ...
  private async getChannels(definedChannels?: string[]): Promise<string[]> {
    ...
    const previouslySelectedChannels: string[] = this.cache
    ...
    let items: vscode.QuickPickItem[] = await this.prepareItems({
      "Default Channels": defaultChannels.map((channel: any) => ({
        label: channel,
        description: "(Added in settings)",
      })),
    ...
    const selectedChannels = await showQuickPick({
