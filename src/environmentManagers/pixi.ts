...
export class Pixi {
  ...
  public async getPythonInterpreterPath(
    ...
    const binPath = path.join(env_info.prefix, "bin", pythonExecutable);
    const rootPath = path.join(env_info.prefix, pythonExecutable);
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Python interpreter not found in ${binPath} or ${rootPath}`
        );
      }
    }
  }
